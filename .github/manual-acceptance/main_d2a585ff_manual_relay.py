...
TASK_UID = "manual-pr63-task"
TASK_TITLE = "Manual PR63 acceptance task"
TRANSCRIPT: list[str] = []
...
def seed_and_configure(home: Path, base_url: str):
    ...
    calendar = principal.make_calendar(name="Manual PR63 Acceptance")
    ...
    calendar.save_todo(
        "\r\n".join(
            [
                "BEGIN:VCALENDAR",
                "VERSION:2.0",
                "PRODID:-//Assistant Operated PR63 Acceptance//EN",
                "BEGIN:VTODO",
                f"UID:{TASK_UID}",
                f"DTSTAMP:{stamp(now)}",
...
def publish_status(seq: int, phase: str, command: str) -> None:
    ...
    body: dict[str, object] = {
        "message": f"Record PR63 manual acceptance status {seq}: {phase}",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="pr63-assistant-operated-") as raw_tmp:
        tmp = Path(raw_tmp)
        home = tmp / "home"
        ...
        try:
            ...
            mark(f"SETUP_READY base_commit=d2a585ff877d2efd092e2be97cc2a58316517a51")
