...
class SettingsActions:
    ...
    def _extensions_status_text(self) -> str:
    ...
    def _cache_status(self) -> dict[str, Any]:
        ...
        if not callable(method):
            raise ValidationError("Cache diagnostics require the production Runtime bridge")
        value = method()
    ...
    def _format_cache_status(status: dict[str, Any]) -> str:
        ...
        if available:
            lines.append(f"Snapshot: Available ({status.get('task_count', 0)} task(s), {status.get('event_count', 0)} event(s))")
            lines.append("Verified from CalDAV: " + str(status.get("synced_at") or "Unknown"))
            if status.get("cache_updated_at"):
                reason = status.get("cache_update_reason") or "local update"
                lines.append(f"Local cache update: {status.get('cache_updated_at')} ({reason})")
        else:
        ...
        if isinstance(counts, dict):
            lines.append(f"Reads since background service start: cache={int(counts.get('cache', 0) or 0)}, CalDAV={int(counts.get('caldav', 0) or 0)}")
    ...
    def _refresh_cache(self) -> dict[str, Any]:
        ...
        if not callable(method):
            raise ValidationError("Cache refresh requires the production Runtime bridge")
        value = method()
        return dict(value) if isinstance(value, dict) else {}

    @staticmethod
    def _format_refresh_result(result: dict[str, Any]) -> str:
        return f"✓ Cache refreshed from authoritative CalDAV: {result.get('task_count', 0)} task(s), {result.get('event_count', 0)} event(s)."
    ...
    def _experimental_panel(self) -> None:
        ...
        while True:
            ...
            selected = self._choose("Experimental", [toggle_label, "Cache status", "Refresh cache now", "How this works"])
            if selected is None:
                return
    ...
    def _caldav_status(self) -> dict[str, Any]:
        ...
        return {"base_url": base_url, "base_url_configured": bool(base_url), "credentials_configured": False}
    ...
    def _set_caldav_credentials(self) -> Any:
        ...
        if callable(method):
            ...
            self.ctx.settings.set(CALDAV_CREDENTIALS, {"username": username, "password": password})
    ...
    def _test_caldav_connection(self) -> Any:
        ...
        if not callable(method):
            raise ValidationError("CalDAV connection test requires the production Runtime bridge")
        result = method()
    ...
    def _caldav_collections(self) -> list[Any]:
        ...
        if not callable(method):
            raise ValidationError("CalDAV collection discovery requires the production Runtime bridge")
    ...
    def _collection_name(item: Any) -> str:
        if not isinstance(item, dict):
            return str(item)
        return str(item.get("name") or item.get("display_name") or item.get("url") or item.get("href") or item)
    ...
    def _choose_collection_role(self, title: str, key: str, component: str, items: list[Any]) -> Any:
        compatible = [item for item in items if component.upper() in self._collection_components(item) and self._collection_url(item)]
        if not compatible:
    ...
    def _collection_roles_panel(self) -> None:
        while True:
            ...
            selected = self._choose("Collection roles", [task_label, event_label, work_label, "Show collections"])
            if selected is None:
                return
            if selected == task_label:
                self._choose_collection_role("Default task collection", CALDAV_TASK_COLLECTION_URL, "VTODO", items)
            elif selected == event_label:
                self._choose_collection_role("Default event collection", CALDAV_EVENT_COLLECTION_URL, "VEVENT", items)
            elif selected == work_label:
                self._choose_collection_role("Work log collection", CALDAV_WORKLOG_COLLECTION_URL, "VEVENT", items)
            elif selected == "Show collections":
                self._show_caldav_collections()

    def _use_discovered_server(self, status: dict[str, Any] | None = None) -> Any:
        status = status or self._caldav_status()
        candidates = [str(item) for item in (status.get("discovered_candidates") or []) if str(item).strip()]
        if not candidates:
    ...
    def _caldav_panel(self) -> None:
        while True:
            ...
            credentials = "Configured" if status.get("credentials_configured") else "Not configured"
            labels = [f"CalDAV server: {server}"]
            if status.get("discovered_candidates"):
                labels.append("Use discovered server")
            labels.extend([f"CalDAV credentials: {credentials}", "Test connection", "Collection roles", "Collections", "Clear credentials"])
    ...
    def _category(self, category: str) -> None:
        ...
        if not specs:
            self._show(f"No editable settings in {category}."); return
        while True:
            labels: list[str] = []
            ...
            for spec in specs:
                ...
                labels.append(label); mapping[label] = spec
    ...
    def list_settings(self, category: str | None = None) -> str:
        ...
        return "\n".join(f"{item['key']} = {_display_value(item.get('value'))}" for item in items)
    ...
    def reset_setting(self, key: str) -> str:
        ...
        if spec.key == CALDAV_CREDENTIALS:
            self._clear_caldav_credentials(); return f"✓ {spec.key} = <hidden>"
        reset = getattr(self.ctx.settings, "reset", None)
        normalized = reset(spec.key) if callable(reset) else self.ctx.settings.set(spec.key, spec.default_value())
        return f"✓ {spec.key} = {_display_value(normalized)}"

    def _caldav_command(self, *parts: Any) -> Any:
        if not parts: return self._caldav_panel()
        action = str(parts[0]).strip().casefold()
        if action == "status": return self._caldav_status()
        if action == "test": return self._test_caldav_connection()
        if action in {"collections", "list"}: return self._show_caldav_collections()
        if action in {"roles", "collection-roles"}:
            if len(parts) != 1: raise ValidationError("settings caldav roles takes no arguments")
            return self._collection_roles_panel()
        if action == "server":
            if len(parts) != 2: raise ValidationError("settings caldav server requires one URL")
            return self._set_caldav_server(str(parts[1]))
        if action in {"credentials", "auth"}:
            if len(parts) != 1: raise ValidationError("Do not put credentials on the command line; use this action interactively.")
    ...
    def _cache_command(self, *parts: Any) -> str:
        ...
        if len(parts) != 1: raise ValidationError("settings cache accepts status or refresh")
        action = str(parts[0]).strip().casefold()
        if action == "status": return self.cache_status_text()
        if action == "refresh": return self._format_refresh_result(self._refresh_cache())
        raise ValidationError(f"Unknown cache settings action: {parts[0]}")

    def _extensions_command(self, *parts: Any) -> Any:
        if not parts: return self._extensions_panel()
        action = str(parts[0]).strip().casefold()
    ...
    def _commands_command(self, *parts: Any) -> Any:
        ...
        if len(parts) != 1: raise ValidationError("settings commands accepts at most one command name")
        return self._run_command("help", str(parts[0]))

    def settings(self, *parts: Any) -> Any:
        if not parts: return self.interactive()
        if not all(isinstance(part, str) for part in parts): raise ValidationError("settings arguments must be text")
        ...
        if action == "categories":
            if len(parts) != 1: raise ValidationError("settings categories takes no arguments")
            return "\n".join(_CATEGORY_ORDER)
        if action == "list":
            if len(parts) > 2: raise ValidationError("settings list accepts at most one category")
            return self.list_settings(parts[1] if len(parts) == 2 else None)
        if action == "get":
            if len(parts) != 2: raise ValidationError("settings get requires one key")
            return self.get_setting(parts[1])
        if action == "set":
            if len(parts) < 3: raise ValidationError("settings set requires KEY VALUE")
            return self.set_setting(parts[1], " ".join(parts[2:]))
        if action == "reset":
            if len(parts) != 2: raise ValidationError("settings reset requires one key")
...
def register_settings_cli_command(commands: CommandService, ctx: Any) -> SettingsActions:
    ...
    if not exists:
        ...
        if callable(register):
            register("settings", actions.settings, description="Open or modify Assistant settings.")
        else:
            registry.register("settings", actions.settings, protected=True, description="Open or modify Assistant settings.")
    return actions
