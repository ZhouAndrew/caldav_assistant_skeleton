...
def build_service_application():
    ...
    registry=CommandRegistry(); commands=CommandService(registry); hooks=HookRegistry(); ctx=AssistantContext(tasks,events,agenda,reminders,notifications,wordpress,HeadlessUI(),temporal,commands,activity,settings,session); _register_builtin_commands(registry,ctx); extensions=ExtensionManager(commands,hooks,settings); _bind_hook_registrar(hooks); dispatcher=RuntimeDispatcher(ctx); background=AssistantService(sync,reminders,wordpress,_ipc_server_for_platform(),dispatcher,PlatformWakeScheduler()); return ServiceApplication(ctx,sync,reminders,wordpress,extensions,background)
